...
- **[R-EMP-DG-001] First-class EmployeeGroupMembership**: Employees owns an **effective-dated** membership table referencing TenantConfig Groups by `GroupId`.
- **[R-EMP-DG-002] HomeWorkArea as a membership role**: Model Home WorkArea as `RoleInGroup=HomeWorkArea`; enforce **exactly one active** HomeWorkArea membership per employee (railroad-scoped or parent-scoped).
- **[R-EMP-DG-003] Scope enforcement via WorkArea subtree**: Actor can access/act on an employee if the employee has **any active membership** within the actor’s allowed WorkArea subtree (descendants resolved via TenantConfig).
- **[R-EMP-DG-004] TreeVersion-aware caching**: Subtree caches keyed by `(parentId, rootGroupId, treeVersion)`; invalidate on `tenantconfig.group.tree.changed`.
- **[R-EMP-DG-005] No cross-DB reads**: Employees integrates with TenantConfig via gRPC/app interfaces + events only.
...
- **EmployeeOrgMemberships** – **first-class** effective-dated `EmployeeGroupMembership` + invariants (HomeWorkArea); provides scope filtering helpers.
...
[EmployeeProfile] --> [EmployeeOrgMemberships]
[EmployeeOrgMemberships] --> [EmployeePool]
...
[EmployeeOrgMemberships] --> [TenantConfig]
...
-- Dynamic Group memberships (TenantConfig integration)
-- Home WorkArea is represented as RoleInGroup='HomeWorkArea' (exactly one active per employee).
CREATE TABLE EmployeeGroupMembership (
  EmployeeGroupMembershipId UNIQUEIDENTIFIER PRIMARY KEY,
  ParentId UNIQUEIDENTIFIER NOT NULL REFERENCES Parent(ParentId),
  EmployeeId UNIQUEIDENTIFIER NOT NULL REFERENCES Employee(EmployeeId),
  GroupId UNIQUEIDENTIFIER NOT NULL, -- TenantConfig GroupId (no FK across module)
  RailroadId UNIQUEIDENTIFIER NULL REFERENCES Railroad(RailroadId), -- NULL = Parent-scoped (roving)
  RoleInGroup NVARCHAR(32) NOT NULL, -- HomeWorkArea/Assigned/Supervisor/Member
  IsPrimary BIT NOT NULL DEFAULT 0,
...
-- Exactly one active HomeWorkArea per employee
CREATE UNIQUE INDEX UX_EGM_Employee_ActiveHome
ON EmployeeGroupMembership(ParentId, EmployeeId)
WHERE RoleInGroup = 'HomeWorkArea'
  AND IsPrimary = 1
  AND EffectiveToUtc IS NULL;

CREATE INDEX IX_EGM_Group_Effective
ON EmployeeGroupMembership(ParentId, GroupId, EffectiveFromUtc, EffectiveToUtc)
INCLUDE (EmployeeId, RoleInGroup, IsPrimary, RailroadId);
...
- Target audiences expressed as **TenantConfig Group Query DSL** (SPEC-1). Employees treats the DSL as opaque and resolves `groupIds` via `EvaluateGroupQuery`.
...
- Employees resolves employeeIds via `EmployeeGroupMembership` (effective-dated at send time) and snapshots the resolved audience per delivery for audit. In-app only for MVP.
...
### Dynamic Groups Integration (SPEC-1 alignment)

- Use TenantConfig read APIs only: `GetGroupDescendants`, `GetTreeVersion`, `IsWorkAreaGroup`, `EvaluateGroupQuery`.
- Cache subtree expansions by `(parentId, rootGroupId, treeVersion)`; invalidate on `tenantconfig.group.tree.changed`.
- Membership mutations (`SetHomeWorkArea`, add/end membership) are serialized per employee using `sp_getapplock` on `Employees:Membership:{parentId}:{employeeId}`.
- All mutating endpoints accept optional `request_id` for idempotent retries.

    ...
    : Repository<CrewPosition>(dbContext, currentUserService), ICrewPositionRepository
{
    ...
    public async Task<List<ControlNumber>> GetVacantStaffablePositionCtrlNbrsAsync(CancellationToken ct = default) =>
        await (from p in DbContext.Set<CrewPosition>()
               where !DbContext.Set<Domain.Modules.Staffing.PositionAssignment>()
                   .Any(a => a.StaffablePositionCtrlNbr == p.StaffablePositionCtrlNbr)
               select p.StaffablePositionCtrlNbr)
            .ToListAsync(ct);
}

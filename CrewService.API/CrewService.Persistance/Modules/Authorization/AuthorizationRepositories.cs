    ...
    : Repository<Permission>(dbContext, currentUserService), IPermissionRepository
{
    public async Task<Permission?> GetByRoleFeatureParentCraftAsync(ControlNumber roleCtrlNbr, ControlNumber featureCtrlNbr, ControlNumber? parentCtrlNbr, ControlNumber? craftCtrlNbr, CancellationToken ct = default)
    {
        return await DbContext.Set<Permission>()
            .SingleOrDefaultAsync(p =>
                ...
                p.ParentCtrlNbr == parentCtrlNbr &&
                p.CraftCtrlNbr == craftCtrlNbr, ct);
    }
    ...
    /// <summary>
    /// Returns the effective permissions for a role in a given parent + craft context.
    /// 4-tier resolution per feature (most specific wins):
    /// 1. craft + parent specific
    /// 2. craft global (no parent)
    ...
    public async Task<List<Permission>> GetEffectivePermissionsAsync(ControlNumber roleCtrlNbr, ControlNumber? parentCtrlNbr, ControlNumber? craftCtrlNbr, CancellationToken ct = default)
    {
        var allForRole = await DbContext.Set<Permission>()
            .Where(p => p.RoleCtrlNbr == roleCtrlNbr &&
                        ...
                        (p.CraftCtrlNbr == null || p.CraftCtrlNbr == craftCtrlNbr))
            .ToListAsync(ct);

        return allForRole
            .GroupBy(p => p.FeatureCtrlNbr)
            .Select(g =>
                g.FirstOrDefault(p => p.CraftCtrlNbr == craftCtrlNbr && p.ParentCtrlNbr == parentCtrlNbr)
                ?? g.FirstOrDefault(p => p.CraftCtrlNbr == craftCtrlNbr && p.ParentCtrlNbr == null)
                ?? g.FirstOrDefault(p => p.CraftCtrlNbr == null && p.ParentCtrlNbr == parentCtrlNbr)
                ?? g.First())
            .ToList();
    }
}

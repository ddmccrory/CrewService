    ...
    : BaseGrpcClient<WorkManagementSrvc.WorkManagementSrvcClient>(channelProvider, tokenProvider, callInvoker => new WorkManagementSrvc.WorkManagementSrvcClient(callInvoker), logger)
{
    ...
    public async Task<GetTemplatePositionsResponse> GetTemplatePositionsAsync(long assignmentTemplateCtrlNbr)
    {
        try
        {
            return await _client.GetTemplatePositionsAsync(new GetTemplatePositionsRequest
            {
    ...
    public async Task<TemplatePositionResponse> CreateTemplatePositionAsync(long assignmentTemplateCtrlNbr, long positionRoleCtrlNbr, int quantity)
    {
        try
        {
            return await _client.CreateTemplatePositionAsync(new CreateTemplatePositionRequest
            {
                AssignmentTemplateCtrlNbr = assignmentTemplateCtrlNbr,
    ...
    public async Task<TemplatePositionResponse> UpdateTemplatePositionAsync(long ctrlNbr, long positionRoleCtrlNbr, int quantity)
    {
        try
        {
            return await _client.UpdateTemplatePositionAsync(new UpdateTemplatePositionRequest
            {
                CtrlNbr = ctrlNbr,
                PositionRoleCtrlNbr = positionRoleCtrlNbr,
                Quantity = quantity
    ...
    public async Task<DeleteResponse> DeleteTemplatePositionAsync(long ctrlNbr)
    {
        try
        {
            return await _client.DeleteTemplatePositionAsync(new DeleteTemplatePositionRequest { CtrlNbr = ctrlNbr });
        }
        catch (Exception ex)
        {
            LogException(ex);
